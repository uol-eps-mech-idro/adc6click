...
class TestAD7214Driver(unittest.TestCase):
    ...
    def test_read_data_wait(self):
        ...
        self.ad7124.set_channel(
            AD7124RegNames.CH0_MAP_REG, enable=True, setup=0, ainp=1, ainm=0
        )
    ...
    def test_to_voltage(self):
        ...
        # 0v unipolar
        expected = 0.0
        value = self.ad7124.to_voltage(
            int_value=0, gain=1, vref=2.50, bipolar=False, scale=1.0
        )
        self.assertAlmostEqual(expected, value, 5)
        # Mid scale unipolar
        expected = 1.25
        value = self.ad7124.to_voltage(
            int_value=0x800000, gain=1, vref=2.50, bipolar=False, scale=1.0
        )
        self.assertAlmostEqual(expected, value, 5)
        # Full scale unipolar
        expected = 2.50
        value = self.ad7124.to_voltage(
            int_value=0xFFFFFF, gain=1, vref=2.50, bipolar=False, scale=1.0
        )
        self.assertAlmostEqual(expected, value, 5)
        # Max -ve bipolar
        expected = -1.25
        value = self.ad7124.to_voltage(
            int_value=0, gain=1, vref=1.25, bipolar=True, scale=1.0
        )
        self.assertAlmostEqual(expected, value, 5)
        # 0v bipolar
        expected = 0.0
        value = self.ad7124.to_voltage(
            int_value=0x800000, gain=1, vref=1.25, bipolar=True, scale=1.0
        )
        self.assertAlmostEqual(expected, value, 5)
        # Max +ve bipolar
        expected = +1.25
        value = self.ad7124.to_voltage(
            int_value=0xFFFFFF, gain=1, vref=1.25, bipolar=True, scale=1.0
        )
        self.assertAlmostEqual(expected, value, 5)
        # Max +ve bipolar with gain
        expected = +1.25 / 16.0
        value = self.ad7124.to_voltage(
            int_value=0xFFFFFF, gain=16, vref=1.25, bipolar=True, scale=1.0
        )
        self.assertAlmostEqual(expected, value, 5)
        # Max +ve bipolar scaled to +/-12.5V
        expected = -12.5
        value = self.ad7124.to_voltage(
            int_value=0x0, gain=1, vref=1.25, bipolar=True, scale=10.0
        )
        self.assertAlmostEqual(expected, value, 5)
        expected = +12.5
        value = self.ad7124.to_voltage(
            int_value=0xFFFFFF, gain=1, vref=1.25, bipolar=True, scale=10.0
        )
        self.assertAlmostEqual(expected, value, 5)
